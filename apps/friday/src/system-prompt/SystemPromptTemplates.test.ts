...
it.effect('renders the channel agent system prompt from thread context and configured models', () =>
  Effect.gen(function* () {
    ...
    assert.include(prompt, '### Shape tasks before starting them')
    assert.include(prompt, 'decisions already made, checkable acceptance criteria')
    assert.include(prompt, 'Resolve choices that affect correctness')
    assert.include(prompt, 'start a narrow investigation that returns those facts')
    assert.include(prompt, 'Do not pass unresolved judgment through vague phrases')
    assert.include(prompt, 'Reserve full verification and expensive analysis')
