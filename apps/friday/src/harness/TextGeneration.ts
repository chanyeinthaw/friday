...
export class TextGenerationError extends Schema.Error<TextGenerationError>('TextGenerationError')({
  ...
  operation: Schema.Literals(['thread-title', 'linked-handoff']),
...
export const LinkedHandoffGeneration = Schema.Struct({
  title: Schema.String,
  prompt: Schema.String.pipe(Schema.check(Schema.isTrimmed(), Schema.isNonEmpty())),
})
export type LinkedHandoffGeneration = typeof LinkedHandoffGeneration.Type

export interface GenerateLinkedHandoffInput {
...
export interface TextGenerationContract {
  ...
  readonly generateLinkedHandoff: (
    input: GenerateLinkedHandoffInput,
  ) => Effect.Effect<LinkedHandoffGeneration, TextGenerationError>
}
