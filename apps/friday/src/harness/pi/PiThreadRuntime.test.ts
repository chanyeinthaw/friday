...
it.effect('replaces active-turn authorization from each steering request before Pi delivery', () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      let currentAuthorization: () => {
        readonly externalUpdateRequests: 'allowed' | 'denied'
      } = () => ({ externalUpdateRequests: 'denied' })
      const session: PiAgentSessionContract = {
        sessionId: 'pi-session-steering-authorization',
        sessionManager: { getSessionFile: () => undefined },
        subscribe: () => () => undefined,
        bindExtensions: async () => undefined,
        prompt: async (text, options) => {
          const authorization = currentAuthorization().externalUpdateRequests
          observed.push({ text, authorization })
          if (options?.streamingBehavior === 'steer') return
          Effect.runFork(Deferred.succeed(turnStarted, undefined))
          await Effect.runPromise(Deferred.await(finishTurn))
        },
        abort: async () => {
          await Effect.runPromise(Deferred.succeed(finishTurn, undefined))
        },
        reload: async () => undefined,
        dispose: () => undefined,
        getSessionStats: () => emptyStats('pi-session-steering-authorization'),
      }
      const runtime = yield* makePiThreadRuntime({
        thread: reloadThread('thread-steering-authorization'),
        sessionFactory: (_thread, authorization) =>
          Effect.sync(() => {
            currentAuthorization = authorization
            return session
          }),
      })
      const turnId = decodeTurnId('turn-steering-authorization')
      const turn = yield* runtime
        .prompt({
          turnId,
          message: { source: 'agent', content: { text: 'imported handoff', images: [] } },
          authorization: { externalUpdateRequests: 'denied' },
          mode: 'turn',
        })
        .pipe(Effect.forkScoped)
      ...
      yield* runtime.prompt({
        turnId,
        message: { source: 'user', content: { text: 'participant approves update', images: [] } },
        authorization: { externalUpdateRequests: 'allowed' },
      })
      yield* runtime.prompt({
        turnId,
        message: { source: 'agent', content: { text: 'synthetic redirect', images: [] } },
        authorization: { externalUpdateRequests: 'denied' },
      })
      yield* runtime.prompt({
        turnId,
        message: { source: 'user', content: { text: 'participant re-approves', images: [] } },
        authorization: { externalUpdateRequests: 'allowed' },
      })

      assert.deepStrictEqual(observed, [
        { text: 'imported handoff', authorization: 'denied' },
        { text: 'participant approves update', authorization: 'allowed' },
        { text: 'synthetic redirect', authorization: 'denied' },
        { text: 'participant re-approves', authorization: 'allowed' },
      ])
      yield* Deferred.succeed(finishTurn, undefined)
      yield* Fiber.join(turn)
      assert.strictEqual(currentAuthorization().externalUpdateRequests, 'denied')
    }),
  ).pipe(Effect.provide(BunCrypto.layer)),
)
